...
var clients = make(map[int]Ping_ChatServiceServer, 10)
...
func sendToStream(csi_ Services_ChatServiceServer, clientUniqueCode_ int, errch_ chan error, is *ChatServer) {
	...
	for {
		...
		for {
			...
			err := clientUniqueCode_.Send(&FromServer{Name: senderName4Client, Body: message4Client, LogTime: timeFromClient})
